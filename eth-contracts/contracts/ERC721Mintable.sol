...
contract Ownable {
    ...
    event OwnershipIsTransfered(address owner);
    ...
    modifier onlyOwner() {
        require(msg.sender == _owner, "The Caller is not the owner");
        _;
    }
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function _setTokenURI(uint256 tokenId) internal {
        require(_exists(tokenId), "TokenId exists");
        string memory convertedTokenID = uint2str(tokenId);
        string memory URI = strConcat(_baseTokenURI, convertedTokenID);
...
contract CustomERC721Token is ERC721Metadata{
    string private _name = 'OhoraToken';
    string private _symbol = 'OHT';
